...
---------------------------------------------------
- Updated imports and async/await usage for python-telegram-bot v20.5+
- Reads BOT_TOKEN from environment and raises a clear error if missing
- No schedulers; animations use edit_message_text + asyncio.sleep
- All original game logic (acts, morgue/flood/archivist arcs, endings) preserved
- Optional OpenAI NPC support via OPENAI_API_KEY
"""
...
# telegram imports updated for v20+
...
async def npc_reply(npc_name: str, player_text: str, sess: Session) -> str:
    ...
    if USE_OPENAI:
        try:
            import openai  # local import to avoid hard dependency when disabled
...
async def main():
    ...
    await app.start()
    try:
        # run_polling wraps initialize/start when used synchronously; here we used async lifecycle
